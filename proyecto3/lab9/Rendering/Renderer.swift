...
final class Renderer: NSObject, MTKViewDelegate {
    ...
    private var models: [ModelAsset] = [
        ModelAsset(
            ...
            textureBaseName: "BambooShoots_Tex",
            preRotateDegXYZ: SIMD3<Float>(0, -40, 0),
            position: SIMD3<Float>(-2, -1, -2.5),
            scale: SIMD3<Float>(repeating: 0.5)
        ),
        ModelAsset(
            name: "homing thistle",
            subdir: "Resources/Models",
            textureBaseName: "Homing_Thistle_Low_homing_thistle_AlbedoTransparency",
            preRotateDegXYZ: SIMD3<Float>(0, 0, 0),
            position: SIMD3<Float>(0, -1, -2.5),
            scale: SIMD3<Float>(repeating: 0.5)
        ),
        ModelAsset(
            name: "Lighting Reed",
            subdir: "Resources/Models",
            textureBaseName: "lightning_reed_exp_lightning_reed_AlbedoTransparency",
            preRotateDegXYZ: SIMD3<Float>(0, 40, 0),
            position: SIMD3<Float>(2, -1, -2.5),
            scale: SIMD3<Float>(repeating: 0.5)
        ),
        ModelAsset(
            name: "Peashooter",
            subdir: "Resources/Models",
            textureBaseName: "Peashooter_Tex",
            preRotateDegXYZ: SIMD3<Float>(0, 180, 0),
            position: SIMD3<Float>(0, -0.7, 0),
            scale: SIMD3<Float>(repeating: 0.5)
        ),
        ModelAsset(
            name: "stage",
            subdir: "Resources/Models",
            textureBaseName: "stage_tex",
            preRotateDegXYZ: SIMD3<Float>(0, 180, 0),
            position: SIMD3<Float>(0, 0, 0),
            scale: SIMD3<Float>(repeating: 2.5)
        )
    ]
    ...
    func draw(in view: MTKView) {
        ...
        if let pipeline = pipeline, let depthState = depthState {
            enc.setRenderPipelineState(pipeline)
            enc.setDepthStencilState(depthState)
            enc.setCullMode(.none)
            enc.setFrontFacing(.counterClockwise)

            for (idx, asset) in models.enumerated() {
                guard let cached = loadModelIfNeeded(index: idx) else { continue }
